/*
 * SEFTestLargeSource.java: JUnit test for the StandardTokenizer
 *
 ...
 * @author  Heiko Blau
 */
public class SEFTestLargeSource extends TestCase {
...
//    String[]   tests = { SEFTestLargeSource.class.getName() };
//    TestUtilities.run(tests, args);
	  
	  TestSuite ts = (TestSuite) SEFTestLargeSource.suite();
	  for (int i = 0; i < ts.countTestCases(); i++) {
		  ...
		  System.out.println("Error count on SEFTestLargeSource: " + tr.errorCount());
  ...
  public static Test suite() {
    ...
    TestSuite suite = new TestSuite(SEFTestLargeSource.class.getName());
    ...
    for (int flagsIndex = 0; flagsIndex < flags.length; ++flagsIndex) {                        
      for (int typesIndex = 0; typesIndex < types.length; ++typesIndex) {                        
        suite.addTest(new SEFTestLargeSource("parseFile", flags[flagsIndex], types[typesIndex]));
        if (charSequenceAvailable) {
          suite.addTest(new SEFTestLargeSource("parseCharSequence", flags[flagsIndex], types[typesIndex]));
        }
      }
    }
   ...
   * Default constructor.
   */  
  public SEFTestLargeSource(String test, int flags, long typeMask) {
    super(test);
    _flags    = flags;
    _typeMask = typeMask;
  }
