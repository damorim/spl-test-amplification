/*
 * SEFTestTextAccess.java: JUnit test for the InputStreamTokenizer
 *
 ...
 * @author  Heiko Blau
 */
public class SEFTestTextAccess extends TestCase {
...
//    String[]   tests = { SEFTestTextAccess.class.getName() };
//
//    TestUtilities.run(tests, args);
    TestSuite ts = (TestSuite) SEFTestTextAccess.suite();
	  for (int i = 0; i < ts.countTestCases(); i++) {
		  ...
		  System.out.println("Error count on SEFTestTextAccess: " + tr.errorCount());
  ...
  public static Test suite() {
    TestSuite   suite = new TestSuite(SEFTestTextAccess.class.getName());
    
    suite.addTest(new SEFTestTextAccess("testGetText"));
    suite.addTest(new SEFTestTextAccess("testSetReadPos"));
    return suite;
  }
   ...
   */  
  public SEFTestTextAccess(String test) {
    super(test);
  }
