...
// #if CLEANERDAEMON
	...
	public void envConfigUpdate(DbConfigManager cm) throws DatabaseException {
		...
		if (nThreads != threads.length) {
			...
			for (int i = 0; i < nThreads; i += 1) {
				if (threads[i] == null) {
					threads[i] = new FileProcessor(name + '-' + (i + 1), env,
							this, profile, fileSelector);
				}
			}
		}
	...
	private void traceFileNotDeleted(Exception e, long fileNum) {
		// #if LOGGINGSEVERE
		Tracer.trace(env, "Cleaner", "deleteSafeToDeleteFiles", "Log file 0x"
	...
	private void processPendingLN(LN ln, DatabaseImpl db, byte[] key,
		...
		try {
			...
			parentFound = tree.getParentBINForChildLN(location, key, dupKey,
					ln, false, true, true, UPDATE_GENERATION);
			bin = location.bin;
	 ...
	 */
	private void migrateLN(DatabaseImpl db, long lsn, BIN bin, int index,boolean wasCleaned,boolean isPending, long lockedPendingNodeId, String cleanAction)
			throws DatabaseException {
		boolean obsolete = false;
		...
		try {
			...
			// #if LOGGINGCLEANER
			trace(detailedTraceLevel, cleanAction, ln, lsn, completed,
					obsolete, migrated);
			// #endif
		}
	}
	...
	private void migrateDupCountLN(DatabaseImpl db, long lsn, DIN parentDIN,
			ChildReference dclRef,boolean wasCleaned,boolean isPending, long lockedPendingNodeId, String cleanAction)
			throws DatabaseException {
		boolean obsolete = false;
		boolean migrated = false;
		boolean lockDenied = false;
	 ...
	 */
	void trace(Level level, String action, Node node, long logLsn,
			boolean completed, boolean obsolete, boolean dirtiedMigrated) {
		// #if LOGGINGBASE
