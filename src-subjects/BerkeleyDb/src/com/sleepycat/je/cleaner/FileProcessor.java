...
class FileProcessor extends DaemonThread {
	 ...
	 * 
	 * @param invokedFromDaemoncurrently
	 *            has no effect.
	 * @param cleanMultipleFilesis
	 *            true to clean until we're under budget, or false to clean at
	 *            most one file.
	 * @param forceCleaningis
	 *            true to clean even if we're not under the utilization
	 *            threshold.
		...
		while (true) {
			...
			try {
				...
				String traceMsg = "CleanerRun "
						...
						+ " nINsCleaned=" + nINsCleanedThisRun + " nINsDead="
						+ nINsDeadThisRun + " nINsMigrated="
						+ nINsMigratedThisRun + " nLNsObsolete="
						+ nLNsObsoleteThisRun + " nLNsCleaned="
						+ nLNsCleanedThisRun + " nLNsDead=" + nLNsDeadThisRun
						+ " nLNsMigrated=" + nLNsMigratedThisRun
						+ " nLNsMarked=" + nLNsMarkedThisRun + " nLNQueueHits="
	...
	/**
	 * Process all log entries in the given file. Note that we check for
	 * obsolete entries using the active TFS (TrackedFileSummary) for a file
	 * while it is being processed, and we prohibit flushing (eviction) of that
	 * offset information until file processing is complete. An entry could
	 * become obsolete because: 1- normal application activity deletes or
	 * updates the entry, 2- proactive migration migrates the entry before we
	 * process it, or 3- if trackDetail is false. However, checking the TFS is
	 * expensive if it has many entries, because we perform a linear search.
	 * There is a tradeoff between the cost of the TFS lookup and its benefit,
	 * which is to avoid a tree search if the entry is obsolete. Note that many
	 * more lookups for non-obsolete entries than obsolete entries will
	 * typically be done. In spite of that we check the tracked summary to avoid
	 * the situation where eviction does proactive migration, and evicts a BIN
	 * that is very soon afterward fetched during cleaning.
	 * 
	...
	private boolean processFile(Long fileNum) throws DatabaseException,
		...
		try {
			...
			// #if LOOKAHEADCACHE
			{
				...
				// #endif
				// #endif
				processLN(fileNum, location, null, null, lookAheadCache,
						dbCache);
			}
			// #endif
			// #endif
	...
	private void processLN(Long fileNum, TreeLocation location, Long offset,
		...
		try {
			...
			boolean parentFound = tree.getParentBINForChildLN(location, key,
					dupKey, ln, false, true, false, Cleaner.UPDATE_GENERATION);
			bin = location.bin;
			...
			// #if LOGGINGCLEANER
			{
				cleaner.trace(cleaner.detailedTraceLevel, Cleaner.CLEAN_LN, ln,
						logLsn, completed, obsolete, false);
			}
			// #endif
			// #endif
		}
	 ...
	 * 
	 * @param infoidentifies
	 *            the LN log entry.
	 * @param logLsnis
	 *            the LSN of the log entry.
	 * @param treeLsnis
	 *            the LSN found in the tree.
	 * @param binis
	 *            the BIN found in the tree; is latched on method entry and
	 *            exit.
	 * @param indexis
	 *            the BIN index found in the tree.
	 * @param parentDINis
	 *            non-null for a DupCountLN only; if non-null, is latched on
	...
	private IN findINInTree(Tree tree, DatabaseImpl db, IN inClone, long lsn)
		...
		try {
			result = tree.getParentINForChildIN(inClone, true,
					Cleaner.UPDATE_GENERATION, inClone.getLevel(), null);
