...
public class DbScavenger extends DbDump {
	...
	public DbScavenger(Environment env, PrintStream outputFile,
	...
	public void dump() throws IOException, DatabaseException {
		...
		envImpl.getFileManager().setLastPosition(nextAvailableLsn, lastUsedLsn,
	...
	private void scavengeDbTree(long lastUsedLsn, long nextAvailableLsn)
		...
		final ScavengerFileReader scavengerReader = new ScavengerFileReader(
				envImpl, readBufferSize, lastUsedLsn, DbLsn.NULL_LSN,
				nextAvailableLsn) {
	...
	private boolean checkProcessEntry(LogEntry entry, LogEntryType entryType,
			boolean pass2) {
		// #if TRANSACTIONS
		...
		if (entry instanceof LNLogEntry) {
			...
			boolean isDelDupLN = entryType
					.equals(LogEntryType.LOG_DEL_DUPLN_TRANSACTIONAL)
