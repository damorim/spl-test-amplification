 ...
 * facility. There are three log handlers set up for logging -- the database log
 * itself, an output file, and stdout (the "console"). By default, only the
 * database file is enabled.
 */
public class Tracer
...
// #endif
{
	 ...
	 */
	public static void trace(EnvironmentImpl envImpl, String sourceClass,
			String sourceMethod, String msg, Throwable t) {
		// #if LOGGINGBASE
		envImpl.getLogger().logp(Level.SEVERE, sourceClass, sourceMethod,
	...
	public static Level parseLevel(EnvironmentImpl envImpl,
			ConfigParam configParam) throws DatabaseException {
		Level level = null;
		try {
			...
			throw new DatabaseException("Problem parsing parameter "
					+ configParam.getName() + ": " + e.getMessage(), e);
		}
		return level;
	}
	...
	// #if LOGGINGDBLOGHANDLER
	/**
	 * @see LoggableObject#marshallOutsideWriteLatchCan be marshalled outside
	 *      the log write latch.
	 */
