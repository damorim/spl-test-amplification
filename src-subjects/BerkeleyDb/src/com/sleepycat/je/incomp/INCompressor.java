...
public class INCompressor extends DaemonThread {
	...
	public synchronized void verifyCursors() throws DatabaseException {
		...
		while (it.hasNext()) {
			...
			DatabaseImpl db = env.getDbMapTree().getDb(binRef.getDatabaseId(),
					lockTimeout, dbCache);
		...
		try {
			...
			if (index >= 0) {
				...
				if (node.containsDuplicates()) {
					...
					bin.releaseLatch();
					// #endif
					duplicateBin = (DBIN) tree.searchSubTree(duplicateRoot,
							dupKey, SearchType.NORMAL, -1, null, false);
					return duplicateBin;
				} else {
					return bin;
				}
	...
	private boolean findDBAndBIN(BINSearch binSearch, BINReference binRef,
		...
		binSearch.db = dbTree.getDb(binRef.getDatabaseId(), lockTimeout,
				dbCache);
