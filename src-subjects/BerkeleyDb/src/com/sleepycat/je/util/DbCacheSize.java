 ...
 * possible, but compression depends on on-disk layouts. DbCacheSize is an aid
 * for estimating cache sizes. To get an estimate of the in-memory footprint for
 * a given database, specify the number of records and record characteristics
 * and DbCacheSize will return a minimum and maximum estimate of the cache size
 * required for holding the database in memory. If the user specifies the
 * record's data size, the utility will return both values for holding just the
 * internal nodes of the btree, and for holding the entire database in cache.
 ...
 * up the btree sizes, and then add 10 percent. Note that the utility does not
 * yet cover duplicate records and the API is subject to change release to
 * release. The only required parameters are the number of records and key size.
 * Data size, non-tree cache overhead, btree fanout, and other parameters can
 * also be provided. For example: $ java DbCacheSize -records 554719 -key 16
 * -data 100 Inputs: records=554719 keySize=16 dataSize=100 nodeMax=128
 * density=80% overhead=10% Cache Size Btree Size Description --------------
 * -------------- ----------- 30,547,440 27,492,696 Minimum, internal nodes only
 * 41,460,720 37,314,648 Maximum, internal nodes only 114,371,644 102,934,480
 * Minimum, internal nodes and leaf nodes 125,284,924 112,756,432 Maximum,
 * internal nodes and leaf nodes Btree levels: 3 This says that the minimum
 * cache size to hold only the internal nodes of the btree in cache is
 * approximately 30MB. The maximum size to hold the entire database in cache,
 * both internal nodes and datarecords, is 125Mb.
 */
public class DbCacheSize {
	private static final NumberFormat INT_FORMAT = NumberFormat
			.getIntegerInstance();
	...
	public static void main(String[] args) {
		try {
			...
			printCacheSizes(System.out, records, keySize, dataSize, nodeMax,
					density, overhead);
			if (measureDir != null) {
	...
	private static void printCacheSizes(PrintStream out, long records,
			int keySize, int dataSize, int nodeMax, int density, long overhead) {
		out.println("Inputs:" + " records=" + records + " keySize=" + keySize
				+ " dataSize=" + dataSize + " nodeMax=" + nodeMax + " density="
				+ density + '%' + " overhead="
				+ ((overhead > 0) ? overhead : 10) + "%");
	...
	// #if MEMORYBUDGET
	private static int calcInSize(int nodeMax, int nodeAvg, int keySize,
	...
	private static void measure(PrintStream out, File dir, long records,
			int keySize, int dataSize, int nodeMax, boolean randomKeys)
	...
	private static Database openDatabase(Environment env, int nodeMax,
			boolean allowCreate) throws DatabaseException {
	...
	private static void insertRecords(PrintStream out, Environment env,
			Database db, long records, int keySize, int dataSize,
	...
	private static void preloadRecords(final PrintStream out, final Database db)
			throws DatabaseException {
	...
	// #if STATISTICS
	private static void printStats(PrintStream out, Environment env, String msg)
			throws DatabaseException {
		out.println();
		out.println(msg + ':');
		EnvironmentStats stats = env.getStats(null);
		out.println("CacheSize="
				+ INT_FORMAT.format(stats.getCacheTotalBytes()) + " BtreeSize="
