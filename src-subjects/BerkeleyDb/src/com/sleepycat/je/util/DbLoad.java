...
public class DbLoad {
	...
	static private DbLoad parseArgs(String argv[]) throws IOException,
			DatabaseException {
	...
	private void loadConfigLine(String line) throws DatabaseException {
		...
		if (keyword.equals("version")) {
			...
			if (!value.equals("3")) {
				throw new IllegalArgumentException("Version " + value
						+ " is not supported.");
			}
			...
			if (value.equals("print")) {
				...
				throw new IllegalArgumentException(value
						+ " is an unknown value for the format keyword");
			}
			if (DEBUG) {
			...
			if (value.equals("true") || value.equals("1")) {
				...
				throw new IllegalArgumentException(value
						+ " is an unknown value for the dupsort keyword");
			}
			if (DEBUG) {
	...
	static private byte backSlashValue = (byte) (new Character('\\')
			.charValue() & 0xff);
