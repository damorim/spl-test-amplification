...
public class Txn extends Locker implements LogWritable, LogReadable {
	 ...
	 * @Override
	 */
	LockResult lockInternal(long nodeId, LockType lockType, boolean noWait,
	 ...
	 * IN compressor queue 5. Release all write locks If any step of this fails,
	 * we must convert this transaction to an abort.
	 */
	public long commit(byte flushSyncBehavior) throws DatabaseException {
		try {
			...
			try {
				...
				// #if LOGGINGSEVERE
				Tracer.trace(envImpl, "Txn", "commit", "Commit of transaction "
						+ id + " failed", t);
				// #endif
	...
	private long abortInternal(boolean forceFlush, boolean writeAbortRecord)
		...
		try {
			...
			synchronized (this) {
				...
				// #if LOGGINGFINE
				Tracer.trace(Level.FINE, envImpl, "Abort:id = " + id
						+ " numWriteLocks= " + numWriteLocks
						+ " numReadLocks= " + numReadLocks + " openCursors= "
						+ openCursors);
				// #endif
				if (openCursors) {
					throw new DatabaseException("Transaction " + id
		...
		try {
			...
			while (undoLsn != DbLsn.NULL_LSN) {
				...
				if (!alreadyUndone.contains(nodeId)) {
					...
					// #if LATCHES
					try {
						RecoveryManager.undo(Level.FINER,db, location, undoLN, undoEntry.getKey(),
								undoEntry.getDupKey(), undoLsn, abortLsn,
								abortKnownDeleted, null, false);
					} finally {
			...
			// #if LOGGINGSEVERE
			Tracer.trace(envImpl, "Txn", "undo", "for node=" + nodeId + " LSN="
					+ DbLsn.getNoFormatString(undoLsn), e);
			// #endif
			throw e;
		}
	 ...
	 * deleteAndReleaseINs, which gets the TxnManager's allTxns latch. The
	 ...
	 */
	public void setHandleLockOwner(boolean ignore, Database dbHandle,
	 ...
	 * true, it means we're being called from abort(). Caller must invoke with
	 * "this" synchronized.
	 */
 ...
 */
	...
	/**
	 * @see LogReadable#readFromLogIt's ok for FindBugs to whine about id not
	 *      being synchronized.
	 */
