...
public class XAEnvironment extends Environment implements XAResource {
	...
	public void commit(Xid xid, boolean ignore) throws XAException{
		...
		try {
			...
			if (txn == null) {
				throw new XAException("No transaction found for " + xid
						+ " during commit.");
			}
	...
	public void end(Xid xid, int flags) throws XAException {
		...
		try {
			if (DEBUG) {
				System.out.println("Transaction for " + Thread.currentThread()
						+ " is "
						+ environmentImpl.getTxnManager().getTxnForThread());
			}
			Transaction txn = environmentImpl.getTxnManager()
					.unsetTxnForThread();
			if (txn == null) {
	...
	public boolean isSameRM(XAResource rm) throws XAException {
		...
		return environmentImpl == DbInternal
				.envGetEnvironmentImpl((XAEnvironment) rm);
	}
	...
	public void start(Xid xid, int flags) throws XAException {
		...
		try {
			...
			if (flags == XAResource.TMNOFLAGS) {
				if (txn == null) {
					if (DEBUG) {
						System.out.println("Transaction for XID " + xid
								+ " being created");
					}
			...
			if (DEBUG) {
				System.out.println("Setting Transaction for "
