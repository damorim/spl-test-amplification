...
class FSyncManager {
	 ...
	 * thread gets stuck here indefinitely. When a thread comes in, it will find
	 * one of two things. 1. There is no fsync going on right now. This thread
	 * should go ahead and fsync. 2. There is an active fsync, wait until it's
	 * over before starting a new fsync. When a fsync is going on, all those
	 * threads that come along are grouped together as the nextFsyncWaiters.
	 * When the current fsync is finished, one of those nextFsyncWaiters will be
	 * selected as a leader to issue the next fsync. The other members of the
	 * group will merely wait until the fsync done on their behalf is finished.
	 * When a thread finishes a fsync, it has to: 1. wake up all the threads
	 * that were waiting for its fsync call. 2. wake up one member of the next
	 * group of waiting threads (the nextFsyncWaiters) so that thread can become
	 * the new leader and issue the next fysnc call. If a non-leader member of
	 * the nextFsyncWaiters times out, it will issue its own fsync anyway, in
	 * case something happened to the leader.
	 */
