...
abstract public class LogManager {
	...
	public LogManager(EnvironmentImpl envImpl, boolean readOnly)
		...
		// #if CHECKSUM
		doChecksumOnRead = configManager
	 ...
	 */
	public long logForceFlush(LoggableObject item, boolean fsyncRequired)
			throws DatabaseException {
	...
	private long log(LoggableObject item, boolean isProvisional,
			boolean flushRequired, boolean fsyncRequired, boolean forceNewLogFile, long oldNodeLsn)
			throws DatabaseException {
		if (readOnly) {
	...
	protected LogResult logInternal(LoggableObject item, boolean isProvisional,
			...
			boolean marshallOutsideLatch, ByteBuffer marshalledBuffer, UtilizationTracker tracker) 
		...
		try {
			...
			LogBuffer useLogBuffer = logBufferPool.getWriteBuffer(entrySize,
					flippedFile);
			marshalledBuffer = addPrevOffsetAndChecksum(marshalledBuffer,
	...
	private LogEntry getLogEntryFromLogSource(long lsn, LogSource logSource)
		...
		try {
			...
			// #endif
			// #endif
			assert LogEntryType.isValidType(loggableType) : "Read non-valid log entry type: "
					+ loggableType;
			LogEntry logEntry = LogEntryType.findType(loggableType, version)
					.getNewLogEntry();
