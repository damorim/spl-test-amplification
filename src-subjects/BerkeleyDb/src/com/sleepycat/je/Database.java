...
public class Database {
	 ...
	 * doc_src directory.
	 */
	public Sequence openSequence(Transaction txn, DatabaseEntry key,
	...
	public OperationStatus putNoOverwrite(Transaction txn, DatabaseEntry key,
	...
	public OperationStatus putNoDupData(Transaction txn, DatabaseEntry key,
			DatabaseEntry data) throws DatabaseException {
		checkEnv();
		DatabaseUtil.checkForNullDbt(key, "key", true);
		DatabaseUtil.checkForNullDbt(data, "data", true);
	 ...
	 */
	OperationStatus putInternal(Transaction txn, DatabaseEntry key,
 ...
 */
	 ...
	 *             correct transactional semantics without incurring a
	 *             performance penalty on all Database operations. Truncate
	 *             functionality has been moved to
	 *             Environment.truncateDatabase(), which requires that all
	 *             Database handles on the database are closed before the
	 *             truncate operation can execute.
	 */
	...
	int truncateInternal(Locker locker, boolean countRecords)
		...
		boolean operationOk = false;
		try {
			TruncateResult result = envHandle.getEnvironmentImpl().truncate(
					locker, databaseImpl);
	 ...
	 * must be called to release the lock. Called by all primary put and delete
	 * operations.
	 */
	 ...
	 * necessary, so null is never returned. releaseTriggerListWriteLock() must
	 * always be called to release the lock.
	 */
	...
	/**
	 * Releases a lock acquired by calling acquireTriggerListWriteLock(). If the
	 * list is now empty then it is set to null, that is, hasTriggers() will
