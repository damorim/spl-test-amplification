...
//#endif
      ...
      try {
        insertTarget=searchSubTreeSplitsAllowed(rootIN,key,nid,updateGeneration);
      }
 catch (      SplitRequiredException e) {
        continue;
      }
...
//#endif
 ...
 while (!(parent instanceof BIN));
      ...
      while (iter.hasPrevious()) {
        ...
        index=info.index;
        if (child.needsSplitting()) {
          int maxEntriesPerNode=(child.containsDuplicates() ? maxDupTreeEntriesPerNode : maxMainTreeEntriesPerNode);
...
//#if LATCHES
    try {
      subtreeRoot.accountForSubtreeRemoval(inList,
//#if CLEANER
tracker
//#endif
);
