...
public class IN extends Node implements Comparable, LoggableObject, LogReadable {
	...
	protected void init(DatabaseImpl db, byte[] identifierKey,
		...
		setDatabase(db);
		EnvironmentImpl env = (databaseImpl == null) ? null : databaseImpl
				.getDbEnvironment();
		// #if LATCHES
			...
			if (lsn == DbLsn.NULL_LSN) {
				...
				try {
					...
					if (!isEntryKnownDeleted(idx)
						...
						throw new DatabaseException(makeFetchErrorMsg(
								LNFE.toString(), this, lsn, entryStates[idx]));
					}
				} catch (Exception e) {
	...
	protected static long computeArraysOverhead(DbConfigManager configManager)
		...
		return MemoryBudget.byteArraySize(capacity)
				+ (capacity * (2 * MemoryBudget.ARRAY_ITEM_OVERHEAD));
	}
	...
	/**
	 * Find the entry in this IN for which key arg is >= the key. Currently uses
	 * a binary search, but eventually, this may use binary or linear search
	 * depending on key size, number of entries, etc. Note that the 0'th entry's
	 * key is treated specially in an IN. It always compares lower than any
	 * other key. This is public so that DbCursorTest can access it.
	 * 
	 * @param key
	 ...
	 * or it was a duplicate, resp. This returns a failure if there's a
	 ...
	 * releasing/retaking in order to check a child LN. Inserts the argument
	 * ChildReference into this IN. Assumes this node is already latched by the
	 * caller.
	 * 
	 * @param entry
	 *            The ChildReference to insert into the IN.
	 ...
	 * node is already latched by the caller. This seems to only be used by
	 * INTest.
	 * 
	 * @param key
	...
	protected void splitInternal(IN parent, int childIndex, int maxEntries,
		...
		try {
			...
			for (int i = low; i < high; i++) {
				...
				newSibling.setEntry(toIdx++, entryTargets[i], thisKey,
						getLsn(i), entryStates[i]);
				clearEntry(i);
			}
			// #if INCOMPRESSOR
			if (deletedEntrySeen)
			// #if INCOMPRESSOR
			{
				databaseImpl.getDbEnvironment().addToCompressorQueue(binRef,
						false);
			}
			// #endif
			// #endif
	...
	public void verify(byte[] maxKey) throws DatabaseException {
		...
		try {
			...
			// #endif
			// #endif
			Comparator userCompareToFcn = (databaseImpl == null ? null
					: getKeyComparator());
			byte[] key1 = null;
			for (int i = 1; i < nEntries; i++) {
				...
				if (s <= 0) {
					throw new InconsistentNodeException("IN " + getNodeId()
							+ " key " + (i - 1) + " ("
							+ Key.dumpString(key2, 0) + ") and " + i + " ("
							+ Key.dumpString(key1, 0) + ") are out of order");
				}
			}
	...
	void findParent(Tree.SearchType searchType, long targetNodeId,
		...
		if (getNEntries() == 0) {
			...
			if (searchType == Tree.SearchType.NORMAL) {
				result.index = findEntry(
						selectKey(targetMainTreeKey, targetDupTreeKey), false,
						false);
	...
	public long log(LogManager logManager, boolean allowDeltas,
		...
		return logInternal(logManager, allowDeltas, false, proactiveMigration,
				null);
	}
	 ...
	 */
	private void traceSplit(Level level, IN parent, IN newSibling,
			long parentLsn, long myNewLsn, long newSiblingLsn, int splitIndex,
